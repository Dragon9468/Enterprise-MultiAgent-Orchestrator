...
// =========================================================
export default function MetricsV1Page() {
  ...
  // 🔒 LEVEL 7 ACCESS GUARD (Chỉ dành riêng cho Ban Quản trị Tối cao - User Level 7)
  if (userRole !== null && userRole < 7) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-8 text-center space-y-4 select-none min-h-[70vh]">
        <div className="p-4 rounded-3xl bg-amber-500/10 border border-amber-500/30 text-amber-500 shadow-xl">
          <Shield className="w-16 h-16 animate-pulse" />
        </div>
        <div className="space-y-1.5">
          <h2 className="text-xl font-bold text-foreground">Khu Vực Tuyệt Mật — Chỉ số V1</h2>
          <p className="text-xs sm:text-sm text-muted-foreground max-w-md mx-auto">
            Ứng dụng <strong>Chỉ số V1 (Bản sao lưu nguyên bản)</strong> chỉ dành riêng cho Ban Quản trị Tối cao (Level 7).
          </p>
        </div>
      </div>
    )
  ...
  return (
    <div className="flex-1 flex flex-col w-full h-full relative overflow-hidden select-none">
      ...
      <div className="flex items-center justify-between border-b border-border/80 px-3 py-1.5 sm:px-6 bg-card/60 backdrop-blur-md sticky top-0 z-20 w-full flex-shrink-0 h-10">
        ...
        {/* 🔒 Level 7 Archive Vault Badge */}
        <div className="flex items-center gap-2">
          <span className="px-2.5 py-1 rounded-lg bg-amber-500/15 border border-amber-500/30 text-amber-400 text-[11px] font-bold flex items-center gap-1.5 shadow-xs">
            <Archive className="w-3.5 h-3.5 text-amber-400" />
            <span className="hidden sm:inline">Chỉ số V1 (Bản lưu trữ gốc - Level 7)</span>
            <span className="sm:hidden">Chỉ số V1 (L7)</span>
          </span>
        </div>
      </div>
